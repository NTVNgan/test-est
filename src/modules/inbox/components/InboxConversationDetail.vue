<template>
    <router-link
        v-if="routeName === 'InboxGroups'"
        :to="{ name: 'InboxGroups', params: { name: groupName, conversation: item.id } }"
        class="box-item-conversation items-center flex px-16p py-18p text-14 text-neutrals-900"
        role="menuitem"
        @click="clickSubmenu(item)"
        :class="checkConvoRemoved ? 'convo-removed' : ''"
    >
        <div class="pr-12p">
            <avatar-component
                :labelShow="false"
        ...
        <div class="box-conversation-info" :class="!item.isSeen ? 'not-seen' : ''">
            ...
            <template v-if="!showIndicatorTyping && !checkUserTypingNote">
                ...
                <template v-else>
                    <template v-if="index !== 0">
                        <template v-if="!checkSeenByMe(item)">
                            <div v-if="!item.last_message.is_whisper" class="last-message flex h-20">
    ...
    <InboxConversationDetailSearch
        ...
        :index="index"
        @clickSubmenuInboxSearch="clickSubmenu"
    >
    </InboxConversationDetailSearch>
</template>
...
import InboxConversationDetailSearch from '@/modules/inbox/components/InboxConversation/InboxConversationDetailSearch.vue';

export default {
    name: 'InboxConversationDetail',
    components: {
        AvatarComponent,
        TooltipComponent,
        InboxConversationDetailSearch,
    ...
    setup(props: any, { emit }: any) {
        ...
        const clickSubmenu = (item: any) => {
            emitter.emit('changeConversation', item);
            emit('updateMenuActive', props.index);
        };
        const { t } = useI18n();
        ...
        const checkConvoRemoved = computed(() => {
            if (props.item.id === cacheConvoRemove.value.id && cacheConvoRemove.value.action !== 'handleInGroupAll') return true;
            if (cacheConvoResolveAndReopen.value.length > 0) {
                const findIndex = cacheConvoResolveAndReopen.value.findIndex(
                    (convo: any) => convo.convo_id === props.item.id && convo.action !== 'someone_reopen',
                );
                return findIndex > -1;
            }
            return false;
...
.box-list-conversation {
    ...
    .box-conversation-info.not-seen {
        .last-message,
        .time-content {
            color: #141417;
            font-weight: 500;
...
<style>
.box-conversation-info .last-message .mention,
.box-conversation-info .last-message span.composer-contenteditable__mention {
    background: none;
    color: inherit;
}
</style>
