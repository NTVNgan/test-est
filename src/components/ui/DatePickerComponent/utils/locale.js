...
import { isDate, isNumber, isString, isObject, isArray, has, defaultsDeep, clamp } from './_';
...
export default class Locale {
    ...
    format(date, mask) {
        ...
        mask = mask.replace(token, $0 =>
            $0 in formatFlags ? formatFlags[$0](this.getDateParts(date), this) : $0.slice(1, $0.length - 1),
        );
    ...
    getDateParts(date) {
        ...
        let tzDate = date;
        if (this.timezone) {
            const normDate = new Date(date.toLocaleString('en-US', { timeZone: this.timezone }));
    ...
    getDateFromParts(parts) {
        ...
        if (this.timezone) {
            const dateString = `${pad(y, 4)}-${pad(m, 2)}-${pad(d, 2)}T${pad(hrs, 2)}:${pad(min, 2)}:${pad(
                sec,
                2,
            )}.${pad(ms, 3)}`;
