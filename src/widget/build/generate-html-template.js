/*
  This script is wrote as a plugin to support Rollup to auto-generate HTML file into production
*/
...
export default function htmlTemplate(options = {}) {
  const { template, target, prefix, suffix, attrs, replaceVars } = options;
  const scriptTagAttributes = attrs && attrs.length > 0 ? attrs : [];
  return {
    name: "html-template",

    async generateBundle(outputOptions, bundleInfo) {
      var bundleKeys = Object.keys(bundleInfo);
      return new Promise(async (resolve, reject) => {
        try {
          ...
          // Inject the script tags before the body close tags
          /* Only build the first main.js */
          bundleKeys = [bundleKeys[0]]
              ...
              .map(
                b =>
                  `<script ${scriptTagAttributes.join(
                    " "
                  )} src="${bundleDirString}${prefix || ""}${b}${suffix}"></script>\n`
              ),
