<template>
    <Dropdown>
        <template #header>
            <div ref="dropdownButtonEl" class="flex items-center mr-4p" @click="toggleDropdown">
                <span class="w-56 mr-4p truncate text-right text-16 font-medium text-neutrals-900">
                    {{ roleName }}
                </span>
                <icon-component name="chevronDown" color="neutrals-500" />
            </div>
        </template>
        <template #list>
            <teleport to="#select">
                ...
                >
                    <list-component v-model="valueSync" class="pb-0">
                        <list-item-component
                            ...
                            :value="item._id"
                        >
                            <span class="text-neutrals-900 font-normal text-16">{{
                                $t(`common.role_${item.name.toLowerCase()}`)
                            }}</span>
                            <template #decription>
                                <span class="text-neutrals-300">{{
                                    $t(`common.role_description_${item.name.toLowerCase()}`)
                                }}</span>
                            </template>
                        </list-item-component>
                    </list-component>
    ...
    setup(props, { emit }) {
        ...
        const setPositionDropdown = () => {
            if (dropdownEl.value && dropdownButtonEl.value) {
                ...
                dropdownLeft.value = dropdownButtonEl.value.getBoundingClientRect().left;
                if (rangeBottomToScreen < dropdownEl.value.getBoundingClientRect().height) {
                    ...
                    dropdownTop.value = dropdownButtonEl.value.getBoundingClientRect().bottom + 8;
