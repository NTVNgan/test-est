<template>
    ...
    <teleport to="#select">
        <span
            ...
            class="dropdown__content break-words w-full fixed text-14 z-999 block text-left font-body"
            :style="[
                {
        ...
        >
            <span
                class="dropdown__inner rounded relative block bg-white text-neutrals-900"
                :style="[
                    {
                        borderRadius: borderRadiusContent,
                    },
                ]"
                :class="{ [contentClass]: true, 'backdrop-filter-blur': blured }"
            >
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const dropdownChild = ref<HTMLElement>();
        const rightDropdown = ref<number | string>('auto');
        const leftDropdown = ref<number | string>('auto');
        const topDropdown = ref<number | string>('auto');
        const bottomDropdown = ref<number | string>('auto');
        const isClickInsideContent = ref<boolean>(false);
        ...
        const getPositionParent = async () => {
            ...
            let DropdownWidth: any = 0;
            let DropdownHeight: any = 0;
            let justifyY = 0;
            let justifyX = 0;
            await nextTick();
            if (dropdownChild.value && dropdownParent.value) {
        ...
        const showDropdown = (action: string) => {
            if (!props.disabled) {
